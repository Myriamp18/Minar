...
function PatioMLE() {
  ...
  useEffect(() => {
    fetch('http://localhost:8081/mpmle')
      .then(res => res.json())
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8081/deletempmle/${id}`)
      .then(res => {
  ...
  return (
    ...
    <div className="text-center">
      <Link to="/creatempmle" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Link to={`/updatempmle/${d.id}`} className='btn btn-warning'>
