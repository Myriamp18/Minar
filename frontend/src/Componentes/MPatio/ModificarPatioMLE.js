...
function ModificarPatioMLE() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    axios.put(`http://localhost:8081/updatempmle/${id}`, values)

        .then(res => {
            console.log(res);
            // Optionally, you can navigate to a different page or update the UI
            navigate('/mpmle'); // Example: Navigate to the home page
        })
        .catch(err => console.log(err));
};
useEffect(() => {
  axios.get(`http://localhost:8081/getrecordmpmle/${id}`)
      .then((res) => {
...
export default ModificarPatioMLE
