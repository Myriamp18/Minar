...
function TrituradaMLE() {
  const [data, setData] = useState([]);
  const navigate = useNavigate()
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    fetch('http://localhost:8081/tmle')
      .then(res => res.json())
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8081/deletetmle/${id}`)
      .then(res => {
  ...
  return (
    ...
    <div className="text-center">
      <Link to="/createtmle" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Link to={`/updatetmle/${d.id}`} className='btn btn-warning'>
