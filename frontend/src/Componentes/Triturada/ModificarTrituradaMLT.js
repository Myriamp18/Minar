...
function ModificarTrituradaMLT() {
  const { id } = useParams();
    ...
    const handleSubmit = (e) => {
      e.preventDefault()
      axios.put(`http://localhost:8081/updatetmlt/${id}`, values)

          .then(res => {
              console.log(res);
              // Optionally, you can navigate to a different page or update the UI
              navigate('/tmlt'); // Example: Navigate to the home page
          })
          .catch(err => console.log(err));
  };
  useEffect(() => {
    axios.get(`http://localhost:8081/getrecordtmlt/${id}`)
        .then((res) => {
  ...
  return (
    ...
    <h1>Modificar Triturada MLT:</h1>
    <div className="close-button" onClick={() => navigate('/tmlt')}>
...
export default ModificarTrituradaMLT
