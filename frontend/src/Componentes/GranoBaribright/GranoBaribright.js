...
function GranoBaribright() {
  const [data, setData] = useState([]);
  const navigate = useNavigate()

  useEffect(() => {
    fetch('http://localhost:8081/granobaribright')
      .then(res => res.json())
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8081/deletegranobaribright/${id}`)
      .then(res => {
  ...
  return (
    ...
    <h1>Grano Barbigth:</h1>

    <div className="text-center">
      <Link to="/creategranobaribright" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
...
export default GranoBaribright
