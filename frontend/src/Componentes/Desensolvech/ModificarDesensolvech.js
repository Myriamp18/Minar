...
function ModificarDesensolvech() {
    const { id } = useParams();
    ...
    const handleSubmit = (e) => {
      e.preventDefault()
      axios.put(`http://localhost:8081/updatedesensolvech/${id}`, values)

          .then(res => {
              console.log(res);
              // Optionally, you can navigate to a different page or update the UI
              navigate('/desensolvech'); // Example: Navigate to the home page
          })
    ...
    useEffect(() => {
        axios.get(`http://localhost:8081/getrecorddesensolvech/${id}`)
            .then((res) => {
  ...
  return (
    ...
    <h1>Modificar Desensolve Jigs:</h1>
    <div className="close-button" onClick={() => navigate('/desensolvech')}>
...
export default ModificarDesensolvech
