...
function Desensolvech() {
    const [data, setData] = useState([]);
    const navigate = useNavigate()
    const [searchTerm, setSearchTerm] = useState('');
    
    useEffect(() => {
      fetch('http://localhost:8081/desensolvech')
        .then(res => res.json())
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8081/deletedesensolvech/${id}`)
        .then(res => {
  ...
  return (
    ...
    <h1>Desensolve Jiggs Chino:</h1>

    <div className="text-center">
      <Link to="/createdesensolvech" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Link to={`/updatedesensolvech/${d.id}`} className='btn btn-warning'>
