...
const ReporteDiario = () => {
  ...
  const toggleTable = (table) => {
    // Si la tabla actualmente abierta es la misma que se ha hecho clic, ciérrala
    // De lo contrario, abre la nueva tabla
    setShowTable(showTable === table ? null : table);
  };
  ...
  const handleDeletejigsch = (id) => {
    axios.delete('http://localhost:8081/deletediariojigsch/' + id)
      .then(res => {
        // Actualiza la lista de datos excluyendo el registro eliminado
        const updatedJigsChData = chino.filter(item => item.id !== id);
  ...
  const handleDeletemesas = (id) => {
    axios.delete('http://localhost:8081/deletediariomesas/' + id)
      .then(res => {
        // Actualiza la lista de datos excluyendo el registro eliminado
        const updatedMesasData = mesas.filter(item => item.id !== id);
  ...
  const handleDeletegrano = (id) => {
    axios.delete('http://localhost:8081/deletediariograno/' + id)
      .then(res => {
        // Actualiza la lista de datos excluyendo el registro eliminado
        const updatedGranoData = grano.filter(item => item.id !== id);
    ...
    <div className=''>
      ...
      <div className='jigs'>
        <div className="button-container">
          <button className="big-button" onClick={() => toggleTable('table1')}>
          ...
          <button className="big-buttonch" onClick={() => toggleTable('table2')}>
            <GiManualMeatGrinder size={30} />
            <br />
            JIGG´S CHINOS
          ...
          <button className="big-buttonmesas" onClick={() => toggleTable('table3')}>
            <MdOutlineTableRestaurant size={30} />
            <br />
            MESAS
          </button>

          <button className="big-buttonselc" onClick={() => toggleTable('table4')}>
            <GiMineralPearls size={30} />
            <br />
            SELECCION
          </button>

          <button className="big-buttonpdf" onClick={onPdf}>
