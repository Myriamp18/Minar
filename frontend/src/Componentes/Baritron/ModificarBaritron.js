...
function ModificarBaritron() {
    ...
    const handleSubmit = (e) => {
      e.preventDefault()
      axios.put(`http://localhost:8081/updatebaritron/${id}`, values)

          .then(res => {
              console.log(res);
              // Optionally, you can navigate to a different page or update the UI
              navigate('/baritron'); // Example: Navigate to the home page
          })
          .catch(err => console.log(err));
  };
  useEffect(() => {
    axios.get(`http://localhost:8081/getrecordbaritron/${id}`)
        .then((res) => {
  ...
  return (
    ...
    <h1>Modificar Baritron:</h1>
    <div className="close-button" onClick={() => navigate('/baritron')}>
...
export default ModificarBaritron
