...
function Baritron() {
    ...
    useEffect(() => {
      fetch('http://localhost:8081/baritron')
        .then(res => res.json())
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8081/deletebaritron/${id}`)
        .then(res => {
  ...
  return (
    ...
    <div className="text-center">
      <Link to="/createbaritron" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Link to={`/updatebaritron/${d.id}`} className='btn btn-warning'>
