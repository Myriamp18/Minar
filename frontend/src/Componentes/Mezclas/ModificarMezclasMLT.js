...
function ModificarMezclasMLT() {
  const { id } = useParams();
    ...
    const handleSubmit = (e) => {
      e.preventDefault()
      axios.put(`http://localhost:8081/updatemmlt/${id}`, values)

          .then(res => {
              console.log(res);
              // Optionally, you can navigate to a different page or update the UI
              navigate('/mmlt'); // Example: Navigate to the home page
          })
          .catch(err => console.log(err));
  };
  useEffect(() => {
    axios.get(`http://localhost:8081/getrecordmmlt/${id}`)
        .then((res) => {
  ...
  return (
    ...
    <h1>Modificar Mezclas MLT:</h1>
    <div className="close-button" onClick={() => navigate('/mmlt')}>
...
export default ModificarMezclasMLT
