...
function ModificarMezclasMLE() {
  const { id } = useParams();
    ...
    const handleSubmit = (e) => {
      e.preventDefault()
      axios.put(`http://localhost:8081/updatemmle/${id}`, values)

          .then(res => {
              console.log(res);
              // Optionally, you can navigate to a different page or update the UI
              navigate('/mmle'); // Example: Navigate to the home page
          })
          .catch(err => console.log(err));
  };
  useEffect(() => {
    axios.get(`http://localhost:8081/getrecordmmle/${id}`)
        .then((res) => {
...
export default ModificarMezclasMLE
