...
function MezclasMLE() {
  ...
  useEffect(() => {
    fetch('http://localhost:8081/mmle')
      .then(res => res.json())
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8081/deletemmle/${id}`)
      .then(res => {
  ...
  return (
    ...
    <div className="text-center">
      <Link to="/createmmle" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Link to={`/updatemmle/${d.id}`} className='btn btn-warning'>
