...
function Desecho43() {
  const [data, setData] = useState([]);
    ...
    useEffect(() => {
      fetch('http://localhost:8081/desecho43')
        .then(res => res.json())
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8081/deletedesecho43/${id}`)
        .then(res => {
  ...
  return (
   ...
   <h1>Desecho Grano 4.00:</h1>

   <div className="text-center">
     <Link to="/createdesecho43" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                 ...
                 <tr key={i}>
                   ...
                   <td>
                     <div style={{ display: 'flex', alignItems: 'center' }}>
                       <Link to={`/updatedesecho43/${d.id}`} className='btn btn-warning'>
