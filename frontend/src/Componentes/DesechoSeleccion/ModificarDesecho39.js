...
function ModificarDesecho39() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    axios.put(`http://localhost:8081/updatededesecho39/${id}`, values)

        .then(res => {
            console.log(res);
            // Optionally, you can navigate to a different page or update the UI
            navigate('/desecho39'); // Example: Navigate to the home page
        })
        .catch(err => console.log(err));
};
useEffect(() => {
  axios.get(`http://localhost:8081/getrecorddesecho39/${id}`)
      .then((res) => {
  ...
  return (
    ...
    <h1>Modificar Desecho 3.90-4.00:</h1>
    <div className="close-button" onClick={() => navigate('/desecho39')}>
...
export default ModificarDesecho39
