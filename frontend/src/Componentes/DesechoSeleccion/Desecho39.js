...
function Desecho39() {
  const [data, setData] = useState([]);
    ...
    useEffect(() => {
      fetch('http://localhost:8081/desecho39')
        .then(res => res.json())
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8081/deletedesecho39/${id}`)
        .then(res => {
  ...
  return (
   ...
   <h1>Desecho Grano 3.90-4.00:</h1>

   <div className="text-center">
     <Link to="/createdesecho39" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
       <FontAwesomeIcon icon={faPlus} />Insertar</Link>
   </div>
   <div className="close-button" onClick={() => navigate('/desechoseleccion')}>
                 ...
                 <tr key={i}>
                   ...
                   <td>
                     <div style={{ display: 'flex', alignItems: 'center' }}>
                       <Link to={`/updatedesecho39/${d.id}`} className='btn btn-warning'>
