...
function GranoJigsChino() {
    const [data, setData] = useState([]);
    const navigate = useNavigate()
  
    useEffect(() => {
      fetch('http://localhost:8081/granojigs')
        .then(res => res.json())
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8081/deletegranojigs/${id}`)
        .then(res => {
  ...
  return (
    ...
    <h1>Grano Jiggs Chino:</h1>

    <div className="text-center">
      <Link to="/creategranojigs" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Link to={`/updategranojigs/${d.id}`} className='btn btn-warning'>
