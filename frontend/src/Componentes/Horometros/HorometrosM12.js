...
function HorometrosM12() {
    const [data, setData] = useState([]);
    const navigate = useNavigate()
    const [searchTerm, setSearchTerm] = useState('');

    useEffect(() => {
      fetch('http://localhost:8081/hmesa12')
        .then(res => res.json())
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8081/deletehmesa12/${id}`)
        .then(res => {
  ...
  return (
    ...
    <h1>Horomero Mesas 1Y2:</h1>
    <div className="close-button" onClick={() => navigate('/horometros')}>
        <FontAwesomeIcon icon={faTimes} />
        </div>
    <div className="text-center">
      <Link to="/createhmesa12" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                      ...
                      <tr key={i}>
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            <Link to={`/updatehmesa12/${d.id}`} className='btn btn-warning'>
