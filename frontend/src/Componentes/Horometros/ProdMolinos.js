...
function ProdMolinos() {
    const [data, setData] = useState([]);
    const navigate = useNavigate()
  
    useEffect(() => {
      fetch('http://localhost:8081/hrsmolinos')
        .then(res => res.json())
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8081/deletehrsmolinos/${id}`)
        .then(res => {
  ...
  return (
    ...
    <h1>Produccion Molinos:</h1>
    <div className="close-button" onClick={() => navigate('/horometros')}>
        <FontAwesomeIcon icon={faTimes} />
        </div>
    <div className="text-center">
      <Link to="/createhrsmolinos" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                      ...
                      <tr key={i}>
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            <Link to={`/updatehrsmolinos/${d.id}`} className='btn btn-warning'>
