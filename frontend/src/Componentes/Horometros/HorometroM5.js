...
function HorometroM5() {
    const [data, setData] = useState([]);
    const navigate = useNavigate()
    const [searchTerm, setSearchTerm] = useState('');

    useEffect(() => {
      fetch('http://localhost:8081/hmesa5')
        .then(res => res.json())
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8081/deletehmesa5/${id}`)
        .then(res => {
  ...
  return (
    ...
    <h1>Horomero Mesas 5:</h1>
    <div className="close-button" onClick={() => navigate('/horometros')}>
        <FontAwesomeIcon icon={faTimes} />
        </div>
    <div className="text-center">
      <Link to="/createhmesa5" className="btn btn-danger btn-lg font-weight-bold   text-lg" >
                      ...
                      <tr key={i}>
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            <Link to={`/updatehmesa5/${d.id}`} className='btn btn-warning'>
