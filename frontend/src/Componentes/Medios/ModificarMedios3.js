  ...
  const { id } = useParams()
    const [values, setValues] = useState({
      ...
      const handleSubmit = (e) => {
        ...
        axios.put(`http://localhost:8081/updatemedios3/${id}`, { ...values, saldo: nuevoSaldo })
          .then(res => {
            console.log(res);
            navigate('/medios3');
          })
      ...
      useEffect(() => {
        axios.get(`http://localhost:8081/getrecordmedios3/${id}`)
            .then((res) => {
...
export default ModificarMedios3
